    ...
    <slot name="button" :isActive="superQueryFlag" :isMobile="izMobile" :open="handleOpen" :reset="handleReset">
      <a-tooltip v-if="superQueryFlag" v-bind="tooltipProps" :mouseLeaveDelay="0.2">
        ...
        <template slot="title">
          ...
          <a class="link" @click="handleReset">清空</a>
        </template>
        <a-button-group>
          ...
          <a-col :sm="24" :md="24 - 5">
            <a-empty v-if="queryParamsModel.length === 0" style="margin-bottom: 12px;">
              <div slot="description">
                <span>没有任何查询条件</span>
                <a-divider type="vertical" />
                <a class="link" @click="handleAdd">点击新增</a>
              </div>
            </a-empty>
...
import * as utils from '@tievd/cube-block/src/utils/util'
import { mixinDevice } from '@tievd/cube-block/src/utils/mixin'
import JDate from './j-date.vue'
import JSelectDepart from '../jeecgbiz/j-select-depart'
import JSelectMultiUser from '../jeecgbiz/j-select-multi-user'
import JMultiSelectTag from '../j-multi-select-tag'
    ...
    renderSaveTreeData(item) {
      ...
      item.title = (arg1, arg2) => {
        ...
        if (arg1.dataRef) {
          ...
          return <span class="error-color">Antdv版本不支持</span>
        }
