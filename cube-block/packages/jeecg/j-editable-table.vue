      ...
      <div class="thead" ref="thead">
        <div class="tr" :style="{ width: this.realTrWidth }">
          ...
          <template v-for="col in columns">
            <div v-show="col.type !== formTypes.hidden" class="td" :key="col.key" :style="buildTdStyle(col, true)">
              <span>{{ col.title }}</span>
            </div>
          </template>
        </div>
                        ...
                        <a-input :key="fileKey" :readOnly="true" :value="file.name">
                          <template slot="addonBefore" style="width: 30px">
                            ...
                            <a-tooltip v-else-if="file.status === 'done'" title="上传完成">
                              <a-icon type="check-circle" class="success-color" />
                            </a-tooltip>
                            <a-tooltip v-else title="上传失败">
                              <a-icon type="exclamation-circle" class="error-color" />
                            </a-tooltip>
                          </template>
                    ...
                    <div v-else-if="col.type === formTypes.file" :key="i">
                      <template v-if="uploadValues[id] != null" v-for="(file, fileKey) of [uploadValues[id] || {}]">
                        <div :key="fileKey" style="position: relative;">
                          <a-tooltip v-if="file.status === 'uploading'" :title="`上传中(${Math.floor(file.percent)}%)`">
                            <a-icon type="loading" class="error-color" />
                            <span style="margin-left:5px" class="error-color">{{ file.status }}</span>
                          </a-tooltip>
                          ...
                          <a-tooltip v-else :title="file.name">
                            <a-icon type="paper-clip" class="error-color" />
                            <span style="margin-left:5px" class="error-color">{{ getEllipsisWord(file.name, 5) }}</span>
                          </a-tooltip>
                    ...
                    <div v-else-if="col.type === formTypes.image" :key="i">
                      <template v-if="uploadValues[id] != null" v-for="(file, fileKey) of [uploadValues[id] || {}]">
                        <div :key="fileKey" style="position: relative;">
                          ...
                          <template v-else>
                            <a-icon type="exclamation-circle" class="error-color" @click="handleClickShowImageError(id)" />
                          </template>
                          <template slot="addonBefore" style="width: 30px">
                            <a-tooltip v-if="file.status === 'uploading'" :title="`上传中(${Math.floor(file.percent)}%)`">
                              <a-icon type="loading" />
...
import { ACCESS_TOKEN } from '@tievd/cube-block/src/store/mutation-types'
import { FormTypes, VALIDATE_NO_PASSED } from '@tievd/cube-block/src/utils/JEditableTableUtil'
import { cloneObject, randomString, randomNumber, getEventPath } from '@tievd/cube-block/src/utils/util'
import JDate from './j-date'
import { filterDictText, initDictOptions } from '@tievd/cube-block/src/utils/packages/JDictSelectUtil'
import { getFileAccessHttpUrl } from '@tievd/cube-block/src/api/manage'
import JInputPop from './minipop/j-input-pop'
import JFilePop from './minipop/j-file-pop'
