    ...
    <template v-if="hasFile">
      <template v-for="(file, fileKey) of [innerFile || {}]">
        <div :key="fileKey" style="position: relative;">
          ...
          <template v-else>
            <a-icon type="exclamation-circle" class="exclamation-circle" @click="handleClickShowImageError" />
          </template>
          <template slot="addonBefore" style="width: 30px">
            ...
            <a-tooltip v-else-if="file.status === 'done'" title="上传完成">
              <a-icon type="check-circle" class="check-circle" />
            </a-tooltip>
            <a-tooltip v-else title="上传失败">
              <a-icon type="exclamation-circle" class="exclamation-circle" />
            </a-tooltip>
          </template>
...
import JVxeCellMixins from '../../j-vxe-table/mixins/JVxeCellMixins'
import { ACCESS_TOKEN } from '@tievd/cube-block/src/store/mutation-types'
import JFilePop from '../../jeecg/minipop/j-file-pop'
import JVxeUploadCell from '../../j-vxe-table/components/cells/j-vxe-upload-cell'
