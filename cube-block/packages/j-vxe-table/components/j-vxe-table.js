...
import { initDictOptions } from '@tievd/cube-block/src/utils/packages/JDictSelectUtil'

import { getRefPromise } from '../utils/vxeUtils'
import { getEnhancedMixins, replaceProps } from '../utils/cellUtils'

import JVxeToolbar from './j-vxe-toolbar'
import JVxeSubPopover from './j-vxe-sub-popover'
import JVxeDetailsModal from './j-vxe-details-modal'
import JVxePagination from './j-vxe-pagination'
import { cloneObject, getVmParentByName, pushIfNotExist, randomString, simpleDebounce } from '@tievd/cube-block/src/utils/util'
import { UtilTools } from 'vxe-table/packages/tools/src/utils'
import { getNoAuthCols } from '@tievd/cube-block/src/utils/authFilter'
    ...
    vxeColumns() {
      this._innerColumns.forEach((column) => {
        ...
        if (column.$type === JVXETypes.file || column.$type === JVXETypes.image) {
          if (column.width && column.width.toString().endsWith('px')) {
