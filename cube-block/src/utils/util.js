 ...
 * @param callback
 */
export function validateDuplicateValue(tableName, fieldName, fieldVal, dataId, callback, message = '') {
  if (fieldVal) {
    let params = { tableName, fieldName, fieldVal, dataId }
    api
      .duplicateCheck(params)
      .then((res) => {
        res['success'] ? callback() : callback(message || res['message'])
      })
      .catch((err) => {
 ...
 * @param {*} actionStr
 */
export function hasPermissionAction(actionStr) {
  let permissionList = []
  ...
  if (!permissions.includes(actionStr)) {
    return false
  }
  return true
}
