  ...
  >
    <div :style="{ width: '100%', padding: '10px 16px' }">
      <a-spin :spinning="confirmLoading">
        ...
        <!-- 选择图标 -->
        <icons @choose="handleIconChoose" @close="handleIconCancel" :iconChooseVisible="iconChooseVisible" symbolUrl=//at.alicdn.com/t/font_8d5l8fzk5b87iudi.js></icons>
      </a-spin>
      <a-row :style="{ textAlign: 'right' }">
        <a-button :style="{ marginRight: '8px' }" @click="handleCancel">
          关闭
        </a-button>
        <a-button v-if="!disableSubmit" @click="handleOk" type="primary">确定</a-button>
      </a-row>
    </div>
  </a-drawer>
</template>
...
<script>
import { addPermission, editPermission, queryTreeList } from '@tievd/cube-block/lib/api/api'
...
export default {
  ...
  methods: {
    ...
    validatePerms: debounce(function (rule, value, callback) {
      if (value && value.length > 0) {
        // 校验授权标识是否存在
        validateDuplicateValue('sys_permission', 'perms', value, this.model.id, callback, '该授权标识已存在')
      } else {
        callback()
      }
    }, 500),
