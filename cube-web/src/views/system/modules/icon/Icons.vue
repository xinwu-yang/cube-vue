  ...
  <a-modal
    ...
    <a-tabs>
      ...
      <a-tab-pane tab="自定义图标" key="7">
        <ul>
          <li v-for="icon in icons.cusIcons" :key="icon">
            <cube-icon :type="icon" :title="icon" @click="chooseIcon('cube-' + icon)" :class="{'active':activeIndex === icon}" />
          </li>
        </ul>
      </a-tab-pane>
    </a-tabs>
...
<script>
import cusIcons from '@/config/custom-icons.conf.js'
