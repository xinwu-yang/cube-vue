<template>
  <component :is="`${layoutMode}-layout`" @dynamicRouterShow="dynamicRouterShow">
...
<script>
import Vue from 'vue'
import SideMenuLayout from './modules/SideMenuLayout'
import TopMenuLayout from './modules/TopMenuLayout'
import MobileLayout from './modules/MobileLayout'
...
export default {
  ...
  watch: {
    ...
    device() {
      ...
      if (this.device === 'mobile') {
        this.$store.commit('SET_DEFAULT_SETTING', { layoutMode: 'mobile' })
      } else {
        this.$store.commit('SET_DEFAULT_SETTING', { layoutMode: defaultSetting.layoutMode })
      }
    }
  ...
  methods: {
    ...
    changeTitle(title) {
      let projectTitle = 'Cube 企业级快速开发平台'
      // 首页特殊处理
      if (this.$route.path === indexKey) {
    ...
    routeReload() {
      ...
      this.$store.commit('SET_DEFAULT_SETTING', { multiPage: false })
      this.$nextTick(() => {
        this.$store.commit('SET_DEFAULT_SETTING', { multiPage: true })
...
.tab-layout-tabs.ant-tabs {
  ...
  background-color: @layout-tab-background;
  padding: 0 20px;
